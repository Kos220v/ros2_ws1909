...
_VALUES_FIELDS = [
    ...
    # tachometer — знаковый int32, коммутационные тики (не механические обороты).
    # НЕ использовать tachometer_abs для одометрии с реверсом!
    ("tachometer",         "i32", 1.0,     "",      "Тахометр (знаковый)"),
    ("tachometer_abs",     "i32", 1.0,     "",      "Тахометр (суммарный пробег)"),
]
...
def parse_get_values(payload):
    ...
    Требуется полный стандартный префикс вплоть до fault_code, включая оба
    тахометра. Укороченный ответ отвергается целиком (None), без частичного
    обновления одометрии. Дополнительные поля новых прошивок в конце разрешены.
    ...
    """
    required_length = 2 + sum(2 if field[1] == "i16" else 4 for field in _VALUES_FIELDS)
    if len(payload) < required_length or payload[0] != Comm.GET_VALUES:
        return None
